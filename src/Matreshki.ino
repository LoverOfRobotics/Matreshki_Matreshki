...
int err, errOld, encB = 0, encC = 0, mm, dist_right, dist_left; // Инициализация переменных
int err_bc, up_bc, err_Old_bc, stopp;
float up, gr;

#define B 10 //Объявление пинов моторов
#define B_back 9
#define C 6
#define C_back 5
byte v_global = 50; //Глобальная скорость
float kp = 0.3; //Коэффициент на линию
float kd = 0.5;
...
byte matreshka_code_OK = 1;   //1, 2, 3, 4   Собственная цифра матрешки
...
void setup() {
  ...
  pinMode(B,OUTPUT);
  pinMode(B_back,OUTPUT);//Присвоение типа к пинам
  pinMode(C,OUTPUT);
  pinMode(C_back,OUTPUT);
  pinMode(A0,INPUT);
  pinMode(A1,INPUT);
  attachInterrupt(digitalPinToInterrupt(3),counterB,RISING);//Аппаратное прерыванеи
  ...
  // lineP(v_global+8);
  // while (s2()>70){
  //   set_speed(0,v_global);
  // }
  // stop(0,v_global);

  lineG(v_global+2,900);
  lineP(v_global+2);
...
int s1(){
  int values1 = map(analogRead(A0), 70, 400, 100, 0);
...
int s2(){
  int values2 = map(analogRead(A1), 50, 400, 100, 0);
